...
# HÀM CHẠY NGẦM: NGỦ 30S RỒI GÓI LOG GỬI ĐI
def process_and_send_background():
    # 1. Ngủ đúng 30 giây để hứng thêm các log khác nếu có
    time.sleep(BUFFER_TIME)
    
    current_time = int(time.time())
    processing_file = f"/var/ossec/logs/gemini_processing_{current_time}.txt"
    
    # 2. Tráo túi và Xóa đồng hồ
    try:
    ...
    # 3. Gọi AI phân tích và báo cáo
    try:
        # Nếu không có file processing (vd: rỗng), thì thoát
        ...
        # Prompt mới: Nhúng tri thức chuyên gia sâu hơn vào hệ thống
...
"""
        ...
        if response.status_code == 200:
            ...
            # FALLBACK: Nếu hết Quota, báo thẳng Telegram
...
# HÀM MAIN: WAZUH KÍCH HOẠT VÀ TẠO TIẾN TRÌNH NGẦM
def main():
    ...
    # Chỉ gom các Alert từ Level 5 trở lên
    if rule_level < 5:
        return
    ...
    # Nếu chưa có tiến trình ngầm nào chạy (không có LOCK_FILE) thì đánh dấu và kích hoạt 1 tiến trình ngầm duy nhất
    # CHỐT HẠ: Kích hoạt 1 tiến trình ngầm duy nhất
    if not os.path.exists(LOCK_FILE):
        try:
            with open(LOCK_FILE, "w") as lf:
                lf.write("running")
            # Dùng sys.executable và sys.argv[0] để gọi đúng môi trường gốc của Wazuh
